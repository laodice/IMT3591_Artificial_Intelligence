...
\begin{algorithm}[htbp]
	\begin{algorithmic}
		\Require{None}
		\Ensure{Stores different important values in a database}
		\Repeat
		\State $data \gets CaptureData()$ 
		\State $data \gets PrepareData(data)$
		\State $data \gets ExtractFeatures()$ \Comment{data: concrete numbers of BP/ BR and HB?}
		\State $median \gets Average(data)$ \Comment{Average of the last 5 minutes}
		\State $weight \gets 1$ 
		\State $GPScoordinate \gets GetGPS()$
		\State $weight \gets 1$  \Comment{Default value for the weighting function}
		\If {OutsideBoundaries(median)}
			\State $response \gets AskUserQuestions()$
			\If $response == yes$ \Comment{//yes $\rightarrow$ valid reason for data out of boundaries}
				\State $ReduceWeight()$
			\Else
		...
		\State $Store(median, weight, GPScooridinate)$ \Comment{Save values in Database}
		\Until{Driving session is over}
		\State $AverageOfSession()$	\Comment{Presented at the end of each driving session}
	\end{algorithmic}
...
\begin{algorithm}[htbp]
	\begin{algorithmic}	
		...
		\Repeat
		\State $data \gets ReadDatabase()$
		\State $rating \gets 0$
		
		\For {Every dataset in data} \Comment{every dataset consists of median, weight and gps}
		\State $rating \gets rating + weight * median$
		\EndFor
		
		\State $rating \gets rating / data.lenght$ \Comment{Average with weight}
		\If {OutsideBoundaries(rating)}
			\State $makeRecommendation()$
		\EndIf
